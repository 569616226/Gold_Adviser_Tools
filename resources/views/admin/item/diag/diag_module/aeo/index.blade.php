                                ...
                                <div class="paddlr20px" style="width: 100%;">
                                    <div class="bigm-card marginT30px">
                                        ...
                                        <div class="am-margin-top">
                                            <h2 class="am-fl">AEO管理
                                                <a href="{{ route('diags.addmod',[$item->id,$diag_mod->id]) }}"
                                                   class="am-margin-left am-btn am-btn-white br5px fs14 {{ Entrust::can('diags.addmod') ? '' : 'am-hide' }}"
                                                   data-am-modal="{target: '#doc-modal-4', closeViaDimmer: 0}">
                                                    <i class="iconfont">&#xe617;</i>
                                                </a>
                                                <a href="javascript:void(0);" name="{{ $diag_mod->id }}"
                                                   class="am-btn am-btn-white br5px {{ Entrust::can('task.set_task_all') ? '' : 'am-hide' }}" onclick="yifpfun(this)">
                                                    一键分配
                                                </a>
                                            </h2>
                                            <div class="am-fr color888">
                                                ...
                                                <a href="javascript:void(0);" name="{{ $diag_mod->id }}"
                                                   class="am-btn am-btn-white br5px am-margin-left {{ Entrust::can('task.settasks') && $is_see ? '' : 'am-hide' }}" onclick="zpfun(this)">指派</a>
                                            </div>
                                            <div style="clear: both;"></div>
                                            <!--小模块-->
                                            @foreach($diag_mod->diag_submods as $diag_submod)

                                                {{--如果是一般贸易类型、则跳过高级贸易类型的内容--}}
                                                @if($item->hands->users->aeo == 0 && $diag_submod->aeo_type == 1 )
                                                    @continue
                                                @endif
                                                ...
                                                <div class="smallm-card  {{ ($diag_submod->task && Auth::guard('admin')->user()->id == $diag_submod->task->admin_user_id)
                                                  || Auth::guard('admin')->user()->id == $diag_mod->fid || $is_see ? '' : 'am-hide' }}">
                                                    ...
                                                    <div class="am-fr color888">
                                                        ...
                                                        <a href="javascript:void(0);" name="{{ $diag_submod->id }}"
                                                           class="am-btn am-btn-primary br5px {{ Auth::guard('admin')->user()->id ==  $item->create_id
                                                           && Entrust::can('task.settask') || $is_see ? '': 'am-hide'}}" onclick="zmokuaifun(this)">分配</a>

                                                        <a href="{{ route('diags.edit',[$item->id,$diag_submod->id]) }}"
                                                           class="am-btn am-btn-primary br5px {{ ($diag_submod->task && Auth::guard('admin')->user()->id == $diag_submod->task->admin_user_id)
                                                           && Entrust::can('diags.edit') ||  Auth::guard('admin')->user()->id == 23 ? '': 'am-hide'}}">编辑</a>
                                                    </div>
                                                    <div style="clear: both;"></div>
                                                    <!--表格（小项）-->
                                                    <table class="am-table am-table-bordered am-table-centered am-margin-top ">
                                                        ...
                                                        <tr class="{{ Entrust::can('diags.addcontent') ? '': 'am-hide' }}">
                                                            <td colspan="5">
                                                                <a href="javascript:void(0);" class="br5px" name="{{ $diag_submod->id }}" onclick="addcontent(this)">
                                                        ...
                                                        @foreach($diag_submod->diag_subcontents as $diag_subcontent)
                                                            <tr>
                                                                ...
                                                                <td class="am-text-middle am-text-left">
                                                                    {{ $diag_subcontent->describle }}
                                                                </td>
                                                                <td class="am-text-middle"><div class="stcha" name="{{ $diag_subcontent->id }}" onclick="flyjshowfun(this)">{{ $diag_subcontent->law }}</div></td>
                                                                <td class="am-text-middle am-text-left" style="max-width: 500px;">
                                                                    {{ $diag_subcontent->suggest }}
                                                                </td>
                                                                <td class="am-text-middle">
                                                                    <a class="{{ Auth::guard('admin')->user()->id == $diag_mod->fid && Entrust::can('diags.editcontent') || $is_see ? '': 'am-hide' }}" href="javascript:void(0);" name="{{ $diag_subcontent->id }}" onclick="editcontent(this)"> 编辑</a>
                                                                    <a class="{{Auth::guard('admin')->user()->id == $diag_mod->fid && Entrust::can('diags.delcontent') || $is_see ? '': 'am-hide' }}" href="javascript:void(0);" name="{{ $diag_subcontent->id }}" onclick="dfcontent(this)" class="colorred"> 删除</a>
                                                                </td>
                                                            </tr>
                                                        @endforeach
                                                        </tbody>
            ...
            <div class="am-modal-bd am-margin-top">
                ...
                <form class="am-form am-form-horizontal" style="padding:20px 40px;" id="zmokuaiform">
                    ...
                    <div class="am-form-group card-box">
                        <label class="am-u-sm-3 am-form-label"><span class="colorred">* </span><span>选择负责人：</span></label>
                        <div class="am-u-sm-8">
                    ...
                    <div class="am-form-group">
                        <div class="am-u-sm-10 am-u-sm-offset-2">
                            <button id="zfpbtn" type="submit" class="am-btn am-btn-primary br5px {{ Entrust::can('task.settask') ? '' : 'am-hide' }}">分配</button>
                        </div>
                    </div>
                </form>
            </div>
        ...
        <div class="am-modal-dialog modalwidth-md">
            ...
            <div class="am-modal-bd rights-modal am-margin-top">
                ...
                <form class="am-form am-form-horizontal" style="padding:20px 40px;" id="addcontentform">
                    <div class="am-form-group">
                        <label class="am-u-sm-2 am-form-label"><span class="colorred">* </span><span>审核内容：</span></label>
                    ...
                    <div class="am-form-group">
                        <div class="am-u-sm-10 am-u-sm-offset-2">
                            <button id="addcontentbtn" type="submit" class="am-btn am-btn-primary br5px {{ Entrust::can('diags.addcontent') ? '' : 'am-hide' }}">提交</button>
                        </div>
                    </div>
                </form>
            </div>
        ...
        <div class="am-modal-dialog modalwidth-md">
            ...
            <div class="am-modal-bd rights-modal am-margin-top">
                ...
                <form class="am-form am-form-horizontal" style="padding:20px 40px;" id="editcontentform">
                    ...
                    <div class="am-form-group">
                        <div class="am-u-sm-10 am-u-sm-offset-2">
                            <button id="editcontentbtn" type="button" class="am-btn am-btn-primary br5px {{ Entrust::can('diags.editcontent') ? '' : 'am-hide' }}">提交</button>
                        </div>
                    </div>
                </form>
            </div>
    ...
    <script>
        ...
        var zpeditform = $("#zpeditform").Validform({
            ...
            beforeSubmit: function (curform) {
                ...
                AjaxJson('/admin/item/'+item_id +'/task/tasks/' + $("#zpbtn").attr("name"), {'taskContent': postDta}, function (data) {
                    if (data.status == 1) {
                        if (data.msg != null && data.msg != undefined) {
                            layer.msg(data.msg)
                        }
