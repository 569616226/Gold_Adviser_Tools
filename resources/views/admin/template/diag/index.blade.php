        ...
        <div class="am-g">
            ...
            <div class="am-u-sm-9 am-u-md-9 am-u-lg-10" id="rightMain">
                <div class="dh-main-container">
                    {{--在线预览通用--}}
                    <p class="title"><a href="{{ route('template.index') }}"><i class="iconfont">&#xe604;</i></a> 诊断报告模板管理</p>
                        ...
                        <div class="paddlr20px">
                            ...
                            <ul class="am-list">
                                @foreach($diags as $diag)
                                    @if(($diag->task && $diag->task->admin_user_id ==  Auth::guard('admin')->user()->id)
                                   ||  (Auth::guard('admin')->user()->id == $item->fid || $item->create_id|| 23))
                                    <li>
                                        <div style="height: 50px;">

                                            @if($diag->title == 1)
                                                <h2 class="am-fl">（一）关于关务风险管理诊断存在问题如下：</h2>
                                            @elseif($diag->title == 2)
                                                <h2 class="am-fl">（二）关于AEO风险管理诊断存在问题如下：</h2>
                                            @elseif($diag->title == 3)
                                                <h2 class="am-fl">（三）关于物流风险管理诊断存在问题如下：</h2>
                                            @elseif($diag->title == 4)
                                                <h2 class="am-fl">（四）关于系统化管理诊断存在问题如下：</h2>
                                            @elseif($diag->title == 5)
                                                <h2 class="am-fl">（五）关于预归类诊断存在问题如下：</h2>
                                            @elseif($diag->title == 6)
                                                <h2 class="am-fl">（六）关于预估价诊断存在问题如下：</h2>
                                            @endif
